...
class GameLogDownloader:
    def __init__(self, end_year: int = 2025, years_count: int = 5):
        # Define the range of years to download (last `years_count` up to `end_year`)
        self.years = list(range(end_year - years_count + 1, end_year + 1))
...
async def main(test: bool = False):
    ...
    downloader = GameLogDownloader(end_year=2025, years_count=5)
