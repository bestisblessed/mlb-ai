...
def fetch_pitcher_details(player_id):
    try:
...
def fetch_all_pitchers_gamelogs(season):
    print("Fetching all MLB teams...")
    teams_data = get_json("teams?sportId=1")

    pitcher_ids = set()
    print("Collecting pitcher IDs from team rosters...")
    ...
    print(f"Found {len(pitcher_ids)} pitchers. Fetching details and game logs...")
    all_records = []
    pitcher_details = []
    
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(fetch_pitcher_details, pid): pid for pid in pitcher_ids}
        futures.update({executor.submit(fetch_pitcher_gamelog, pid, season): pid for pid in pitcher_ids})
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing"):
            ...
            if isinstance(result, list):
                ...
                pitcher_details.append(result)

    df_pitchers = pd.DataFrame(all_records)
    df_pitcher_details = pd.DataFrame(pitcher_details)
    
    os.makedirs(f"data/{season}", exist_ok=True)
    df_pitchers.to_csv(f"data/{season}/pitchers_gamelogs_{season}_statsapi.csv", index=False)
    df_pitcher_details.to_csv(f"data/{season}/pitchers_details_{season}_statsapi.csv", index=False)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        year = 2025
    df_pitchers, df_pitcher_details = fetch_all_pitchers_gamelogs(year)
    print(f"Saved {len(df_pitchers)} pitcher game logs.")
    print(f"Saved {len(df_pitcher_details)} pitcher details.")
    print(df_pitchers.head())
    print(df_pitcher_details.head())
